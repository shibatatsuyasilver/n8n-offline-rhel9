...
```
┌──────────────────────────────┐       ┌──────────────────────────────┐
│  RHEL host A — nginx + n8n   │ ────▶ │  RHEL host B — PG18+pgAdmin4 │
│  HTTPS 443 → 127.0.0.1:5678  │ 5432  │  systemd / postgresql-18     │
│  /etc/n8n/n8n.env + tls cert │       │  pgAdmin HTTPS :5050         │
└──────────────────────────────┘       └──────────────────────────────┘
```
...
| n8n | 2.17.7 |
| Node.js | v22.22.2 |
| PostgreSQL | 18 (PGDG `rhel-9` yum repo, pinned to RHEL 9.2-compatible releases) |
| pgAdmin4 | 9.15 (`pgadmin4-web`, pinned for RHEL 9.2-compatible offline install) |
| pgvector | PGDG `pgvector_18` (安裝後在 n8n DB 自動 `CREATE EXTENSION vector`) |
| 目標 OS | RHEL 9.2 x86_64 |

完整 RPM 清單以各 bundle 內的 `rpm-packages.tsv` 為準。

RPM 來源鎖定為 Rocky Linux 9.2 vault + PGDG `rhel-9` + 官方 pgAdmin RHEL 9 repo。Rocky 9.2 可作為 RHEL 9.2 相容來源，但不要使用浮動的 `rockylinux:9` 產包，否則可能抓到較新版本的 OpenSSL / OpenSSH / systemd 套件。PG18 與 pgAdmin 需要的 `libpq5` 會選「最新 9.2 相容」套件，不追 `rhel9.7` 這類較新 release tag，也會跳過需要 OpenSSL 3.4 symbol 的 RPM。

## 套件清單

> 以下為當前 bundle 版本（n8n 2.17.7 / PostgreSQL 18）的快照；重新打 bundle 後請以 `rpm-packages.tsv` / `npm-packages.json` 為準。

### n8n bundle (`dist/n8n-offline-rhel9.2-x86_64/`)
...
`git` 不再列入系統 RPM seed，因為 RHEL/Rocky 9.2 的 `git-core` 會硬相依 `openssh-clients`。為避免離線安裝替換主機 SSH/OpenSSL 套件，需要 git 時請先在 RHEL 主機用原生 repo 安裝。
...
### PostgreSQL bundle (`dist/postgres-offline-rhel9.2-x86_64/`)

**PostgreSQL 版本**：18（PGDG `rhel-9` yum repo；下載前會過濾 `rhel9.3+` / `el9_3+`，也會跳過需要 OpenSSL 3.4 symbol 的 RPM；實際版本以 `rpm-packages.tsv` 為準）。目前實測會選 `postgresql18* 18.0-1PGDG.rhel9` 與 `pgvector_18 0.8.1-1PGDG.rhel9`；`postgresql18-contrib 18.1-1PGDG.rhel9` 需要 `OPENSSL_3.4.0`，不適合 RHEL 9.2。

**pgAdmin4 版本**：預設 9.15（官方 pgAdmin RHEL 9 yum repo；`prepare-pg-online.sh` 會 pin `pgadmin4-web` / `pgadmin4-server`，並為 RHEL 9.2 選相容的 `libpq5`）。pgAdmin4 只在離線 RHEL host B 的 production install 啟用；Docker no-systemd 驗證模式不啟動 Apache。

PG18 另外需要 `liburing.so.2`，Rocky 9.2 vault 只有舊版 `liburing`，所以 `prepare-pg-online.sh` 會額外 exact-pin `liburing-2.12-1.el9.x86_64` 到暫存 pinned repo，再用這個 pinned repo + Rocky 9.2 解析相依；不會開放浮動 UBI repo 參與整包相依解析。

#### RPM 套件（含遞移相依）

直接安裝（seed，由 `prepare-pg-online.sh` 指定）：`postgresql18-server`, `postgresql18-contrib`, `postgresql18`, `pgvector_18`, `pgadmin4-web`, `mod_ssl`
...
| acl | 檔案 ACL 權限工具 |
| alternatives | 管理系統替代命令連結 |
| apr | Apache 可攜執行階段程式庫 |
| apr-util | APR 工具程式庫 |
| apr-util-bdb | APR Berkeley DB 後端 |
| apr-util-openssl | APR OpenSSL 加密後端 |
| audit-libs | Linux audit 稽核程式庫 |
| basesystem | Rocky/RHEL 基礎系統定義 |
| bash | Shell 指令執行環境 |
| bzip2-libs | bzip2 壓縮格式程式庫 |
| ca-certificates | 信任根憑證集合 |
| checkpolicy | SELinux 政策編譯器 |
| coreutils | 基本檔案與文字命令 |
| coreutils-common | coreutils 共用資料檔 |
| cracklib | 密碼強度檢查程式庫 |
| cracklib-dicts | 密碼檢查字典資料 |
| crypto-policies | 系統加密政策設定 |
| curl | HTTP/HTTPS 傳輸工具 |
| cyrus-sasl-lib | SASL 認證支援程式庫 |
| dbus | 系統服務訊息匯流排 |
| dbus-broker | D-Bus 訊息代理服務 |
| dbus-common | D-Bus 共用設定檔 |
...
| gzip | gzip 壓縮工具 |
| hostname | 顯示/設定主機名稱工具 |
| httpd | Apache HTTP Server，用於 pgAdmin4 web |
| httpd-core | Apache HTTP Server 核心 |
| httpd-filesystem | Apache HTTP Server 目錄結構 |
| httpd-tools | Apache HTTP Server 工具 |
| keyutils-libs | Linux keyring 程式庫 |
| kmod-libs | Kernel module 管理程式庫 |
| krb5-libs | Kerberos 認證程式庫 |
| libacl | ACL 權限程式庫 |
...
| libmount | 檔案系統掛載程式庫 |
| libnghttp2 | HTTP/2 通訊程式庫 |
| libpath_utils | 路徑處理工具程式庫 |
| libpq5 | PostgreSQL 用戶端程式庫 |
| libpsl | Public Suffix List 程式庫 |
| libpwquality | 密碼品質檢查程式庫 |
| libseccomp | 系統呼叫過濾程式庫 |
| libselinux | SELinux 執行時期程式庫 |
| libselinux-utils | SELinux 狀態查詢工具 |
| libsemanage | SELinux 政策管理程式庫 |
| libsepol | SELinux 政策程式庫 |
| libsigsegv | 記憶體錯誤處理程式庫 |
| libsmartcols | 表格輸出格式程式庫 |
...
| libxslt | XSLT 轉換程式庫 |
| libzstd | Zstandard 壓縮程式庫 |
| lua-libs | Lua 執行階段程式庫 |
| lz4-libs | LZ4 壓縮程式庫 |
| mailcap | MIME 類型對應資料 |
| mod_http2 | Apache HTTP/2 模組 |
| mod_lua | Apache Lua 指令稿模組 |
| mod_ssl | Apache TLS/SSL 模組 |
| mpfr | 高精度浮點數程式庫 |
| ncurses | 終端文字介面工具 |
| ncurses-base | 終端能力資料庫 |
| ncurses-libs | 終端控制程式庫 |
...
| pgadmin4-server | pgAdmin4 server runtime |
| pgadmin4-web | pgAdmin4 Apache web mode |
| pgvector_18 | PostgreSQL 向量搜尋擴充 |
| policycoreutils | SELinux 政策管理工具 |
| policycoreutils-python-utils | SELinux 管理工具（semanage） |
| popt | 命令列選項解析程式庫 |
...
| python3-audit | Python audit 稽核綁定 |
| python3-distro | Python 發行版偵測模組 |
| python3-libs | Python 3 標準程式庫 |
| python3-libselinux | Python SELinux 綁定 |
| python3-libsemanage | Python SELinux 政策管理綁定 |
| python3-mod_wsgi | Apache WSGI 模組，用於 pgAdmin4 |
| python3-pip-wheel | pip 安裝工具 wheel |
| python3-policycoreutils | Python SELinux 政策工具 |
| python3-setools | SELinux 政策分析工具 |
| python3-setuptools | Python 套件建置工具 |
| python3-setuptools-wheel | setuptools 建置工具 wheel |
| readline | 互動式命令列編輯庫 |
| redhat-logos-httpd | Red Hat HTTPD 預設頁素材 |
| rpm | RPM 套件管理工具 |
| rpm-libs | RPM 套件管理程式庫 |
| sed | 串流文字編輯工具 |
| setup | 系統基礎設定檔 |
| shadow-utils | 系統帳號管理工具 |
...
- `prepare-online.sh` — 在連網主機產出 **n8n** bundle (`dist/n8n-offline-rhel9.2-x86_64/`)
- `install-offline.sh` — 自動拷貝進 n8n bundle，於離線 RHEL 9 host A 執行
- `prepare-pg-online.sh` — 在連網主機產出 **PostgreSQL 18** bundle (`dist/postgres-offline-rhel9.2-x86_64/`)
- `install-pg-offline.sh` — 自動拷貝進 PG bundle，於離線 RHEL 9 host B 執行
- `verify-offline.sh` — 在開發機用 `docker network --internal` 起 `postgres:18` + UBI 9.2 跨容器跑一次 `install-offline.sh --verify-no-systemd`，端到端驗證
- `verify-pg-offline.sh` — 在開發機用 `docker run --network none`（完全離線）跑 `install-pg-offline.sh --verify-no-systemd --verify-pgadmin`，驗證 PG18 + pgvector + pgAdmin4 全部能從 bundle 離線裝起來並服務（curl `https://:5050/pgadmin4/`）
- `verify-pg-data-preserved.sh` — 用持久化 docker volume 連跑兩次 `install-pg-offline.sh`，證明第二次安裝會偵測到既有資料目錄、跳過 initdb 並保留既有資料
- `verify-pg-n8n-access.sh` — 在 `--internal` 網路 + 固定 IP 容器內跑 `install-pg-offline.sh --verify-no-systemd --verify-n8n-access`，驗證 n8n-ready 自動設定（listen/pg_hba 窄範圍、role/db/owner/pgvector、n8n 遠端建表讀寫）

## 1. 取得 bundle

兩個選項：

### 1a. 直接下載預先建構好的 bundle（離線主機需要 RHEL 9.2 / x86_64）

從 [Releases](https://github.com/shibatatsuyasilver/n8n-offline-rhel9/releases) 取得：

```bash
gh release download v0.1.7 -R shibatatsuyasilver/n8n-offline-rhel9
tar -xf n8n-offline-rhel9.2-x86_64.tar
tar -xf postgres-offline-rhel9.2-x86_64.tar
```

若只下載分割檔，先串回 tar 再解壓縮：

```bash
cat n8n-offline-rhel9.2-x86_64.tar.part-* > n8n-offline-rhel9.2-x86_64.tar
cat postgres-offline-rhel9.2-x86_64.tar.part-* > postgres-offline-rhel9.2-x86_64.tar
shasum -a 256 -c SHA256SUMS-release.txt
tar -xf n8n-offline-rhel9.2-x86_64.tar
tar -xf postgres-offline-rhel9.2-x86_64.tar
```

或用 curl 直接下載 asset URL。SHA-256 在 release notes 內，也可用各 `.parts.SHA256SUMS` 驗證分割檔。

### 1b. 自己從原始碼建構

需要：本機可用 `docker`（拉 `registry.access.redhat.com/ubi9/ubi:9.2` / `postgres:18` 映像，並在容器內 `dnf download` / `npm install`）。RPM 下載來源固定為 Rocky 9.2 vault；PG 來源固定為 PGDG `rhel-9` repo；pgAdmin4 來源固定為官方 pgAdmin RHEL 9 repo，但 `prepare-pg-online.sh` 只會下載 release tag 不高於 RHEL 9.2 的套件。

```bash
./prepare-online.sh        # → dist/n8n-offline-rhel9.2-x86_64/
./prepare-pg-online.sh     # → dist/postgres-offline-rhel9.2-x86_64/
```
...
```bash
TARGET_RHEL_MINOR=9.2
RHEL_IMAGE=registry.access.redhat.com/ubi9/ubi:9.2
ROCKY_VAULT_BASE=https://download.rockylinux.org/vault/rocky/9.2
PGDG_RHEL_MINOR=9
PG_COMPAT_LIBURING_SPEC=liburing-2.12-1.el9.x86_64
PG_COMPAT_RPM_DIR=dist/postgres-offline-rhel9.2-x86_64/rpm-repo
PGADMIN4_VERSION=9.15
PGADMIN_REPO_BASE=https://ftp.postgresql.org/pub/pgadmin/pgadmin4/yum/redhat/rhel-9-x86_64
```

> 2026-06-02 實測：PGDG RHEL 9 yum repo 的 metadata 可能只保留較新的 RHEL minor（例如 `rhel9.8`）套件；若要繼續產 RHEL 9.2 相容的 PG18 bundle，請保留一份已驗證過的 PG18 `rpm-repo/`，並用 `PG_COMPAT_RPM_DIR` 指向它，再讓腳本補抓 pgAdmin4 / Apache 相關 RPM。

重建後請確認 manifest 沒有漂到 9.3+，也沒有 Rocky release identity package。`nginx-core` / `httpd` 需要的 `system-logos-httpd` 由 UBI 的 `redhat-logos-httpd` 提供，不使用 `rocky-logos*`。n8n bundle 也不應包含 OpenSSH 套件，避免離線安裝影響既有 SSH 服務：

```bash
rg '(el9_[3-9]|rhel9[._][3-9])|rocky-(release|repos|gpg-keys|logos)' \
  dist/n8n-offline-rhel9.2-x86_64/rpm-packages.tsv \
  dist/postgres-offline-rhel9.2-x86_64/rpm-packages.tsv
rg '^openssh(-clients|-server)?\t' dist/n8n-offline-rhel9.2-x86_64/rpm-packages.tsv
rg '^(pgadmin4-web|pgadmin4-server|httpd|python3-mod_wsgi|redhat-logos-httpd)\t' \
  dist/postgres-offline-rhel9.2-x86_64/rpm-packages.tsv
```

前兩個檢查應該沒有輸出；最後一個檢查應列出 pgAdmin4 / Apache 相關套件。prepare script 也會在偵測到不相容項目時直接失敗。

重建 PG bundle 後，建議跑離線安裝驗證（皆用 Docker，不需要真的 RHEL 主機）：

```bash
./verify-pg-offline.sh          # --network none 完全離線裝 PG18 + pgvector + pgAdmin4 並 curl :5050/pgadmin4/
./verify-pg-data-preserved.sh   # 連跑兩次安裝，證明既有資料目錄不會被 initdb 覆寫
./verify-pg-n8n-access.sh       # 驗證 n8n-ready 自動設定（listen/pg_hba 窄範圍 + n8n 遠端建表/讀寫）
```

## 2. 離線 host B：安裝 PostgreSQL 18

把 `dist/postgres-offline-rhel9.2-x86_64/` 拷貝到 RHEL 9 host B：

```bash
cd postgres-offline-rhel9.2-x86_64
sudo ./install-pg-offline.sh
```

腳本會：
- 從本地 RPM 倉庫安裝 `postgresql18-server` + `postgresql18-contrib` + `postgresql18` + `pgvector_18` + `pgadmin4-web`
- **僅在 datadir 尚未初始化時**才 `postgresql-18-setup initdb`；若 `/var/lib/pgsql/18/data` 已是既有 cluster，會偵測到並**跳過 initdb、保留既有資料**
- `systemctl enable --now postgresql-18`
- 產生 pgAdmin4 self-signed TLS 憑證並設定 Apache 監聽 `0.0.0.0:5050`
- 建立 pgAdmin4 admin login：`tonystark@local.com`
- 建立 PostgreSQL 管理角色：`tonystark`

> **資料安全**：production 安裝**不會影響既有 PostgreSQL 資料**。安裝腳本只在 datadir 未初始化時才 `initdb`（偵測到 `PG_VERSION`/`base/` 就跳過）；若 `/var/lib/pgsql/18/data` 已存在但不是合法 cluster，腳本會直接中止而非覆寫。未提供 `N8N_HOST_IP` 時**不會**改 `postgresql.conf` / `pg_hba.conf`；提供 `N8N_HOST_IP` 時只會「**新增**」listen IP 與一條 n8n 專用 `pg_hba` 規則（append，不刪既有設定），一樣不影響既有資料。RPM 安裝為平行可裝，不會動到 RHEL 原生 PostgreSQL 13 的 `/var/lib/pgsql/data`。此行為由 `verify-pg-data-preserved.sh` 連跑兩次安裝 + sentinel 驗證。唯一會被改動的是 pgAdmin4 的 `tonystark` 管理角色密碼（若該角色已存在會被 `ALTER` 成安裝時輸入的密碼）。

### 2a. n8n-ready 自動設定（建議：兩台主機部署）

提供 `N8N_HOST_IP` 時，production 安裝會**自動把這台 Postgres 主機設成 n8n 可直接連**，免去下方的手動步驟：

```bash
sudo N8N_HOST_IP='10.46.171.96' \
     POSTGRES_HOST_IP='10.46.171.97' \
     N8N_DB_PASSWORD='<給 n8n role 的密碼>' \
     ./install-pg-offline.sh
```

腳本會（在裝完 PG + pgAdmin4 後）額外：
- `listen_addresses = 'localhost,<POSTGRES_HOST_IP>'`（**永不**用 `'*'`）
- `pg_hba.conf` 新增 `host n8n n8n <N8N_HOST_IP>/32 scram-sha-256`（只放行 n8n 主機）
- 建立 `n8n` role + `n8n` database（owner=n8n）+ 啟用 `pgvector`
- 對 `n8n` 授權：DB owner、`public` schema owner + `USAGE/CREATE`、既有與**未來**物件的完整權限（`ALTER DEFAULT PRIVILEGES`）
- `firewall-cmd` 只放行 `<N8N_HOST_IP>/32` 連 `5432`（`PG_AUTO_FIREWALL=0` 可跳過）
- `systemctl restart postgresql-18` 套用設定，並印出 n8n 主機要用的 `N8N_DB_*` 值

| 變數 | 預設 | 說明 |
|---|---|---|
| `N8N_HOST_IP` | 未設 | 允許連線的 n8n 主機 IP/CIDR；**設了才會啟用本自動流程** |
| `N8N_DB_PASSWORD` | 未設 | `n8n` role 密碼；設了 `N8N_HOST_IP` 時必填 |
| `POSTGRES_HOST_IP` | 自動偵測 | 加進 `listen_addresses` 的本機 IP（`hostname -I` 第一個） |
| `N8N_DB_NAME` / `N8N_DB_USER` | `n8n` / `n8n` | n8n 資料庫名 / role 名 |
| `PG_PORT` | `5432` | firewall/摘要用的 PG port |
| `PG_AUTO_FIREWALL` | `1` | 設 `0` 不改 firewalld |

> 安全性：listen 只綁 `localhost + 指定 IP`、pg_hba 只放行 `<n8n>/32`、firewall 只放行 `<n8n>/32` 連 5432，皆為窄範圍。此自動設定由 `verify-pg-n8n-access.sh` 在 `--internal` 網路內驗證（含 n8n 角色實際遠端建表/寫入/讀取）。不提供 `N8N_HOST_IP` 時維持下方 §2 的手動流程。

pgAdmin4 和 PostgreSQL `tonystark` 角色會使用同一組安裝時輸入的密碼。預設會互動式提示；若需要無人值守，可用環境變數提供，但不要把密碼寫進 shell history 或提交到 repo：

```bash
sudo PGADMIN_ADMIN_PASSWORD='<admin-password>' ./install-pg-offline.sh
```
...
| 變數 | 預設 |
|---|---|
| `PGADMIN_ENABLE` | `1`（設 `0` 可只安裝 PostgreSQL） |
| `PGADMIN_ADMIN_EMAIL` | `tonystark@local.com` |
| `PGADMIN_DB_ADMIN_USER` | `tonystark` |
| `PGADMIN_LISTEN_ADDRESS` | `0.0.0.0` |
| `PGADMIN_HTTPS_PORT` | `5050` |
| `PGADMIN_TLS_CERT_FILE` / `PGADMIN_TLS_KEY_FILE` | 未設定時自動產生 self-signed cert |

pgAdmin4 安裝完成後可用：

```bash
curl -k https://127.0.0.1:5050/pgadmin4/
ss -ltn | grep ':5050'
```
...
```text
https://<host_b_ip>:5050/pgadmin4
```

第一次連線會看到 self-signed 憑證警告。installer **預設會自動開放 firewalld 的 TCP 5050**（`PGADMIN_AUTO_FIREWALL=1`，預設對**所有來源** `0.0.0.0/0`），所以裝完即可從外部瀏覽器連入。

| 變數 | 預設 | 說明 |
|---|---|---|
| `PGADMIN_AUTO_FIREWALL` | `1` | 設 `0` 則不自動開 firewalld（改回手動） |
| `PGADMIN_ALLOW_CIDR` | 空（所有來源） | 設成網段（如 `10.46.171.0/24`）只放行該來源連 5050 |

> pgAdmin4 是資料庫管理介面。預設對外全開只靠登入帳密 + HTTPS 保護；若只需特定管理端連入，安裝時設 `PGADMIN_ALLOW_CIDR` 收窄，或事後手動調整：
>
> ```bash
> sudo firewall-cmd --permanent --remove-port=5050/tcp
> sudo firewall-cmd --permanent --add-rich-rule='rule family="ipv4" source address="<admin_cidr>" port port="5050" protocol="tcp" accept'
...
把 `dist/n8n-offline-rhel9.2-x86_64/` 拷貝到 RHEL 9 host A，提供必填環境變數後執行：

```bash
cd n8n-offline-rhel9.2-x86_64
...
```bash
./verify-offline.sh --bundle-dir dist/n8n-offline-rhel9.2-x86_64
```

機制：建立 `docker network create --internal`（無 internet 出口）→ `postgres:18` 容器 + UBI 9.2 容器互連 → UBI 9.2 內 mount bundle 跑 `install-offline.sh --verify-no-systemd`，模擬 host A 連到外部 PG。
...
兩個 UBI 9.2 容器，一個跑 `install-pg-offline.sh --verify-no-systemd`，另一個跑 `install-offline.sh --verify-no-systemd`，同 internal 網路互連。
