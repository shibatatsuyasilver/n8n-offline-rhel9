...
RPM 來源鎖定為 Rocky Linux 9.2 vault + PGDG `rhel-9`。Rocky 9.2 可作為 RHEL 9.2 相容來源，但不要使用浮動的 `rockylinux:9` 產包，否則可能抓到較新版本的 OpenSSL / OpenSSH / systemd 套件。PG18 會從 PGDG `rhel-9` repo 選「最新 9.2 相容」套件，不追 `rhel9.7` 這類較新 release tag，也會跳過需要 OpenSSL 3.4 symbol 的 RPM。
...
```bash
gh release download v0.1.5 -R shibatatsuyasilver/n8n-offline-rhel9
...
需要：本機可用 `docker`（拉 `registry.access.redhat.com/ubi9/ubi:9.2` / `postgres:18` 映像，並在容器內 `dnf download` / `npm install`）。RPM 下載來源固定為 Rocky 9.2 vault；PG 來源固定為 PGDG `rhel-9` repo，但 `prepare-pg-online.sh` 只會下載 release tag 不高於 RHEL 9.2 的套件。
...
重建後請確認 manifest 沒有漂到 9.3+，也沒有 Rocky release identity package。`nginx-core` 需要的 `system-logos-httpd` 由 UBI 的 `redhat-logos-httpd` 提供，不使用 `rocky-logos*`。n8n bundle 也不應包含 OpenSSH 套件，避免離線安裝影響既有 SSH 服務：
